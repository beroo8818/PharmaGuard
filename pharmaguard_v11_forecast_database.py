...
def _pg_to_number(value):
    ...
    try:
        ...
        return np.nan
# ----------------------------------------------------------------------
# SPECIAL IMPORTER: Daily Monthly Movement Workbook
# Handles workbooks like:
# Sheets: المتنوعات / الاقراص / الامبولات
# Columns: الصنف / رصيد أول المدة / طبيعة الحركة / days 1..31
# Optional inventory sheet: محضر جرد
# ----------------------------------------------------------------------

def _pg_cell_text(value):
    return normalize_search_text(clean_text(value))


def _pg_sheet_has_daily_movement(raw_df):
    if raw_df is None or raw_df.empty:
        return False

    scan = raw_df.head(15).fillna("").astype(str)
    text = " ".join(scan.values.flatten().tolist())
    text_norm = normalize_search_text(text)

    has_item = "الصنف" in text_norm
    has_movement = "طبيعه الحركه" in text_norm or "طبيعة الحركة" in text
    has_opening = "رصيد اول المده" in text_norm or "رصيد اول المدة" in text_norm

    return has_item and has_movement and has_opening


def _pg_is_daily_movement_workbook(path):
    try:
        xl = pd.ExcelFile(path)
    except Exception:
        return False

    target_sheets = ["المتنوعات", "الاقراص", "الأقراص", "الامبولات", "الأمبولات"]
    ...
    for sheet in xl.sheet_names:
        ...
        try:
            ...
            if _pg_sheet_has_daily_movement(raw):
                return True
        except Exception:
            continue
...
def _read_inventory_count_sheet(path):
    stock_map = {}
    ...
    if "محضر جرد" not in xl.sheet_names:
        return stock_map

    try:
        raw = pd.read_excel(path, sheet_name="محضر جرد", header=None)
    except Exception:
        return stock_map
    ...
    for r in range(min(len(raw), 30)):
        row_values = [clean_text(v) for v in raw.iloc[r].tolist()]
        row_norm = [normalize_search_text(v) for v in row_values]

        for c, value in enumerate(row_norm):
            if value == "الصنف":
                item_col = c

            if "رصيد الجرد" in value:
                stock_col = c

        if item_col is not None and stock_col is not None:
            header_row = r
            break
    ...
    for r in range(header_row + 1, len(raw)):
        item_name = clean_text(raw.iat[r, item_col])
        if not item_name:
            continue

        stock_value = _pg_to_number(raw.iat[r, stock_col])
        if pd.isna(stock_value):
            continue

        key = normalize_search_text(item_name)
        stock_map[key] = float(stock_value)

    return stock_map


def import_daily_movement_workbook(path):
    xl = pd.ExcelFile(path)
    ...
    inventory_stock_map = _read_inventory_count_sheet(path)

    rows = []
    report = []
    preview_frames = []

    report.append("Importer selected: DAILY MONTHLY MOVEMENT WORKBOOK.")
    report.append("Detected sheets with daily movement format: الصنف / رصيد أول المدة / طبيعة الحركة / أيام الشهر.")
    ...
    for sheet in xl.sheet_names:
        ...
        try:
            ...
            report.append(f"Skipped sheet {sheet}: {exc}")
            continue

        if not _pg_sheet_has_daily_movement(raw):
            continue

        preview_frames.append(raw.head(12).copy())
        ...
        for r in range(min(len(raw), 20)):
            ...
            for c, value in enumerate(row_norm):
                ...
                if "رصيد اول المده" in value or "رصيد اول المدة" in value:
                    opening_col = c

                if "طبيعه الحركه" in value or "طبيعة الحركة" in row_values[c]:
                    movement_col = c

            if item_col is not None and opening_col is not None and movement_col is not None:
                header_row = r
                break

        if header_row is None:
            report.append(f"Skipped sheet {sheet}: required headers not found.")
            continue

        day_row = header_row + 1

        day_cols = []
        for c in range(movement_col + 1, raw.shape[1]):
            top_value = raw.iat[header_row, c] if header_row < len(raw) else None
            day_value = raw.iat[day_row, c] if day_row < len(raw) else None

            top_num = _pg_to_number(top_value)
            day_num = _pg_to_number(day_value)

            is_day_number = (
                (not pd.isna(day_num) and 1 <= day_num <= 31)
                or (not pd.isna(top_num) and 1 <= top_num <= 31)
            )

            text_value = normalize_search_text(f"{top_value} {day_value}")
            if "total" in text_value or "الرصيد" in text_value or "المنصرف" in text_value:
                continue
        ...
        if not day_cols:
            report.append(f"Skipped sheet {sheet}: no day columns found.")
            continue
        ...
        for r in range(header_row + 2, len(raw)):
            item_name = clean_text(raw.iat[r, item_col])
            movement_type = clean_text(raw.iat[r, movement_col])
            movement_norm = normalize_search_text(movement_type)

            if item_name:
                current_item = item_name
                opening_value = _pg_to_number(raw.iat[r, opening_col])
                current_opening = 0.0 if pd.isna(opening_value) else float(opening_value)

                if current_item not in item_bucket:
                    item_bucket[current_item] = {
                        "opening_stock": current_opening,
                        "dispensed": 0.0,
                        "returned": 0.0,
                        "orders": 0.0,
                    }
            ...
            day_sum = 0.0
            for c in day_cols:
                val = _pg_to_number(raw.iat[r, c])
                if not pd.isna(val):
                    day_sum += float(val)

            if "منصرف" in movement_norm:
                item_bucket[current_item]["dispensed"] += day_sum
            elif "مرتجع" in movement_norm:
                item_bucket[current_item]["returned"] += day_sum
            elif "طلبيه" in movement_norm or "طلبية" in movement_type:
                item_bucket[current_item]["orders"] += day_sum

        for i, (medicine_name, values) in enumerate(item_bucket.items(), start=1):
            if not medicine_name:
                continue

            medicine_key = normalize_search_text(medicine_name)

            opening_stock = float(values.get("opening_stock", 0) or 0)
            monthly_consumption = float(values.get("dispensed", 0) or 0)
            returned_qty = float(values.get("returned", 0) or 0)
            orders_qty = float(values.get("orders", 0) or 0)

            if medicine_key in inventory_stock_map:
                current_stock = inventory_stock_map[medicine_key]
                stock_source = "محضر جرد"
            else:
                current_stock = opening_stock - monthly_consumption + returned_qty + orders_qty
                if current_stock < 0:
                    current_stock = 0
                stock_source = "calculated from opening - dispensed + returned + orders"

            if opening_stock == 0 and monthly_consumption == 0 and current_stock == 0:
                continue

            generic_name, brand_name, form_group, analysis_name = parse_scientific_and_trade_names(
                medicine_name,
                dosage_map.get(sheet, "Miscellaneous")
            )
            ...
            rows.append({
                ...
                "brand_names_display": brand_name,
                "dosage_form_group": dosage_map.get(sheet, "Miscellaneous"),
                "analysis_name": analysis_name,
                "source_medicine_names": medicine_name,
                "item_code": f"DAILY-{sheet}-{i:04d}",
                "current_stock": current_stock,
                "min_stock_level": max(monthly_consumption * 0.5, 0),
                "avg_daily_consumption": monthly_consumption / 30.0 if monthly_consumption else 0,
                "avg_monthly_consumption": monthly_consumption,
                "lead_time_days": 14,
                "pending_po_qty": 0,
                "expiry_date": None,
                "clinical_priority": "Medium",
                "storage_location": "Central Store / دفتر الحركة اليومية",
                "branch_name": "Central Store / دفتر الحركة اليومية",
                "unit_cost": 0,
        ...
        report.append(f"Sheet {sheet}: extracted {len(item_bucket):,} medicines.")

    if not rows:
        raise ValueError(
            "No usable daily movement rows were extracted.\n"
            "Expected sheets like المتنوعات / الاقراص / الامبولات with columns:\n"
            "الصنف / رصيد أول المدة / طبيعة الحركة / days 1..31"
        )

    parsed = pd.DataFrame(rows)

    report.append(f"Rows extracted before normalization: {len(parsed):,}")
    report.append(f"Inventory count rows from محضر جرد: {len(inventory_stock_map):,}")
    report.append("Note: this workbook has no batch number or expiry date. Expiry analysis will remain Unknown until batch/expiry data is imported.")
    ...
    if preview_frames:
        preview = pd.concat(preview_frames, ignore_index=True).head(50)
    else:
        preview = parsed.head(50)

    return normalized, "Daily movement sheets + محضر جرد", "\n".join(report), preview
...
def import_any_file(path):
    ...
    try:
        ...
        try:
            if is_excel and _pg_is_daily_movement_workbook(safe_path):
                df, sheet_used, report, preview = import_daily_movement_workbook(safe_path)
                report = "Importer selected: DAILY MONTHLY MOVEMENT WORKBOOK / دفتر حركة يومية شهري.\n" + str(report)
                return df, sheet_used, report, preview
        except Exception as daily_detect_exc:
            report_notes.append(f"Daily movement importer detection warning: {daily_detect_exc}")
        try:
...
class PharmaGuardMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.refresh_action.triggered.connect(self.refresh_from_monthly_outputs_or_recompute)
    ...
    def _write_import_sync_log(self, message):
        try:
            log_dir = Path("reports")
            log_dir.mkdir(exist_ok=True)
            log_path = log_dir / "import_sync_log.txt"
    ...
    def load_latest_monthly_outputs_as_analytics(self):
        """
        Load latest monthly_outputs/monthly_reports_*.xlsx into Analytics format.
        This lets Executive Dashboard / Monthly Reports data appear inside Analytics.
        """
        monthly_dir = Path("monthly_outputs")

        if not monthly_dir.exists():
            raise FileNotFoundError("monthly_outputs folder was not found.")

        files = sorted(
            monthly_dir.glob("monthly_reports_*.xlsx"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )

        if not files:
            raise FileNotFoundError("No monthly_reports_*.xlsx files were found in monthly_outputs.")

        latest_file = files[0]

        try:
            item_summary = pd.read_excel(latest_file, sheet_name="02_Item_Summary")
        except Exception:
            item_summary = pd.read_excel(latest_file)

        if item_summary is None or item_summary.empty:
            raise ValueError(f"Latest monthly report is empty: {latest_file}")

        def pick_col(df, names, default=""):
            for name in names:
                if name in df.columns:
                    return df[name]
            return pd.Series([default] * len(df), index=df.index)

        scientific_name = pick_col(
            item_summary,
            ["scientific_name", "medicine_name", "generic_name", "item_name"],
            "Unknown Item",
        ).astype(str)

        trade_names = pick_col(
            item_summary,
            ["trade_names", "trade_name", "brand_name", "brand_names_display"],
            "",
        ).astype(str)

        unit = pick_col(
            item_summary,
            ["unit", "dosage_form_group", "form"],
            "Miscellaneous",
        ).astype(str)

        branch = pick_col(
            item_summary,
            ["branch", "branch_name", "storage_location"],
            "Main Store",
        ).astype(str)

        current_stock = pd.to_numeric(
            pick_col(item_summary, ["current_stock", "opening_stock", "stock", "balance"], 0),
            errors="coerce",
        ).fillna(0)

        monthly_consumption = pd.to_numeric(
            pick_col(item_summary, ["avg_monthly_consumption", "total_issued_month", "issued_this_month"], 0),
            errors="coerce",
        ).fillna(0)

        dynamic_min = pd.to_numeric(
            pick_col(item_summary, ["dynamic_min", "dynamic_min_stock", "min_stock_level"], 0),
            errors="coerce",
        ).fillna(0)

        shortage_risk = pick_col(
            item_summary,
            ["shortage_risk", "risk"],
            "Low",
        ).astype(str)
        ...
        normalized_source = pd.DataFrame({
            "medicine_name": scientific_name,
            "generic_name": scientific_name,
            "brand_name": "",
            "brand_names_display": trade_names,
            "dosage_form_group": unit,
            "analysis_name": scientific_name + " - " + unit,
            "source_medicine_names": scientific_name,
            "item_code": [f"MONTHLY-{i+1:05d}" for i in range(len(item_summary))],
            "current_stock": current_stock,
            "min_stock_level": dynamic_min,
            "avg_daily_consumption": monthly_consumption / 30.0,
            "avg_monthly_consumption": monthly_consumption,
            "lead_time_days": 14,
            "pending_po_qty": 0,
            "expiry_date": None,
            "clinical_priority": "Medium",
            "storage_location": branch,
            "branch_name": branch,
            "unit_cost": 0,
            "supplier_name": "Unknown Supplier",
            "supplier_on_time_rate": 0.75,
        ...
        normalized_source = normalize_dataframe(normalized_source)

        return (
            normalized_source,
            f"Latest monthly report: {latest_file.name}",
            item_summary.head(50),
        )

    def refresh_from_monthly_outputs_or_recompute(self):
        """
        Refresh Analytics from latest Executive / Monthly Reports output.
        If no monthly output exists, fallback to normal recompute.
        """
        try:
            df, sheet_used, preview = self.load_latest_monthly_outputs_as_analytics()

            self.df_raw = df
            self.current_sheet = sheet_used
            self.mapping_report_text = (
                "Analytics refreshed from monthly_outputs.\n"
                f"Source: {sheet_used}\n"
                "This sync lets Executive Dashboard imports appear inside Analytics."
            )
            self.preview_df = preview
            self.import_diagnosis_df = build_import_diagnosis(
                self.df_raw,
                self.preview_df,
                self.mapping_report_text,
            )
            self.import_summary_text = build_import_quick_summary(
                self.df_raw,
                self.current_sheet,
                self.mapping_report_text,
            )
            ...
            self.recompute()
            self._write_import_sync_log(f"Executive/Monthly Reports → Analytics sync OK | {sheet_used}")

            QMessageBox.information(
                self,
                t(self.lang, "title"),
                "Analytics refreshed from latest Executive / Monthly Reports file."
            )
            return

        except Exception as exc:
            self._write_import_sync_log(f"Executive/Monthly Reports → Analytics sync skipped | {exc}")
            self.recompute()
    ...
    def import_file_dialog(self):
        ...
        try:
            ...
            self.recompute()
            try:
                from database.analytics_dashboard_bridge import after_analytics_import
                bridge_path = after_analytics_import(self)
                if bridge_path is not None and hasattr(self, "status"):
                    self.status.showMessage(f"Executive Dashboard updated from Analytics import: {bridge_path.name}", 9000)
            except Exception as bridge_exc:
                print("Executive Dashboard bridge failed after Excel import:", bridge_exc)
    ...
    def import_google_sheet_dialog(self):
        ...
        try:
            ...
            try:
                ...
                bridge_path = after_analytics_import(self)
                if bridge_path is not None and hasattr(self, "status"):
                    self.status.showMessage(f"Executive Dashboard updated from Google Sheet import: {bridge_path.name}", 9000)
            except Exception as bridge_exc:
                print("Executive Dashboard bridge failed after Google Sheet import:", bridge_exc)
