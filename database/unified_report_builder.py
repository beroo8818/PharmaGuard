...
def ensure_sample_n8n_config() -> Path:
    if not CONFIG_PATH.exists():
        sample = {"webhook_url": "https://YOUR-N8N-DOMAIN/webhook/pharmaguard-report", "token": "CHANGE_ME_SECRET_TOKEN", "to": "pharmacy.manager@hospital.gov", "subject_prefix": "PharmaGuard", "include_latest_analytics_exports": True}
...
def send_package_to_n8n(package: ReportPackage, config: Optional[Dict[str, object]] = None) -> requests.Response:
    ...
    url, token = str(config.get("webhook_url") or "").strip(), str(config.get("token") or "").strip()
    recipient, subject_prefix = str(config.get("to") or "").strip(), str(config.get("subject_prefix") or "PharmaGuard").strip()
    ...
    html_body = package.summary_html.read_text(encoding="utf-8") if package.summary_html.exists() else ""
    payload = {"report_code": "combined_operational_package", "report_name_ar": "ملخص مجمع + تقرير تشغيلي", "generated_at": package.metrics.get("generated_at"), "to": recipient, "subject": f"{subject_prefix} - ملخص تشغيلي وتقرير Operational", "body_html": html_body, "metrics_json": json.dumps(package.metrics, ensure_ascii=False)}
    headers = {"x-pharmaguard-token": token}
    file_handles = []
    files = {}
    try:
        def add_file(field: str, path: Optional[Path]):
            if not path or not path.exists(): return
            f = open(path, "rb"); file_handles.append(f)
            suffix = path.suffix.lower()
            mime = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" if suffix == ".xlsx" else "image/png" if suffix == ".png" else "text/html" if suffix == ".html" else "application/json" if suffix == ".json" else "application/octet-stream"
            files[field] = (path.name, f, mime)
        add_file("operational_report", package.operational_report)
        add_file("summary_image", package.summary_image)
        add_file("summary_html_file", package.summary_html)
        add_file("report_catalog", package.catalog_path)
        for idx, path in enumerate(package.latest_analytics_files[:3], start=1):
            add_file(f"analytics_export_{idx}", path)
        response = requests.post(url, data=payload, files=files, headers=headers, timeout=90)
